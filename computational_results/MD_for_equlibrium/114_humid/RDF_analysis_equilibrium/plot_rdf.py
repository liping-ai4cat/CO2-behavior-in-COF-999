...
def compute_rdf_parallel(traj_files, atom_pair=('N', 'N'), r_max=10.0, n_bins=200, n_cores=None, last_n_frames=6000):
    """
    Calculates RDF for any pair of atoms using only the LAST n frames of the trajectory.
    """
    elem1, elem2 = atom_pair
    ...
    print(f"Processing last {last_n_frames} frames of: {traj_files}")
    ...
    for traj_file in traj_files:
        try:
            ...
            # --- Logic to select only the last N frames ---
            start_index = max(0, n_frames - last_n_frames)
            frames_to_process = n_frames - start_index
            
            print(f"File {traj_file}: Total {n_frames} frames. Analysis range: {start_index} -> {n_frames} ({frames_to_process} frames)")

            total_files_frames += frames_to_process

            # Divide this specific subset into chunks
            chunk_size = max(1, int(np.ceil(frames_to_process / n_cores)))

            # Loop starts from start_index, not 0
            for i in range(start_index, n_frames, chunk_size):
    ...
    print(f"Total frames to analyze: {total_files_frames}")
    ...
    csv_filename = f'rdf_{elem1}{elem2}_last{last_n_frames}.csv'
    ...
    plt.title(f'RDF (Last {last_n_frames} frames)')

    plt.legend()
    plt.grid(alpha=0.3)
    plt.tight_layout()

    plot_filename = f'rdf_{elem1}{elem2}_last{last_n_frames}.png'
...
if __name__ == "__main__":
    ...
    # Run analysis for last 6000 frames
    compute_rdf_parallel(my_files, atom_pair=('N', 'N'), last_n_frames=10000)
    compute_rdf_parallel(my_files, atom_pair=('N', 'O'), last_n_frames=10000)
    compute_rdf_parallel(my_files, atom_pair=('O', 'O'), last_n_frames=10000)
